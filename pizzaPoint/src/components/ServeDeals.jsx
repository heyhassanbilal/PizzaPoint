  ...
  return (
    <>
        ...
        <div className='mt-7 grid grid-cols-1 justify-items-center gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
          {data.map((item) => (
            item.size == "MEDIUM" ? <CardLong item={item} key={item.id} img={item.imageUrl} title={item.name} price={item.price} description={item.description} size={item.size} id={item.id}/> : null
          ))}
